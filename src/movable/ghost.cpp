...
void Ghost::move(Board_cells* board, int& nb_eaten_gum)
{
    ...
    if (position.y % CELL_SIZE == half_cell_size && position.x % CELL_SIZE == half_cell_size)
    {
        ...
        if (this->open_directions != next_possible_directs)
        {
            this->open_directions = next_possible_directs;
            std::random_device rd;  // a seed source for the random number engine
            std::mt19937 gen(rd()); // mersenne_twister_engine seeded with rd()
            std::uniform_int_distribution<> distrib(0, this->open_directions.size() - 1);
